...
public class ReturnDAO {
    ...
    public List<ReturnRequest> getReturnsByStatus(String status) throws SQLException {
    String sql = "SELECT * FROM returnrequest WHERE status = ?";
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        while (rs.next()) {
            list.add(mapResultSetToReturnRequest(rs));
        }
    }
    return list;
}
    ...
    public List<ReturnRequest> getPendingReturnsWithProductName() throws SQLException {
    String sql = "SELECT r.*, p.name AS productName FROM returnrequest r " +
                 "JOIN product p ON r.productId = p.productId " +
    ...
    public boolean deleteReturnRequest(int requestId) throws SQLException {
        String sql = "DELETE FROM returnrequest WHERE requestId = ? AND status = 'pending'";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, requestId);
            return ps.executeUpdate() > 0;
    ...
    public int countReturnsByStatus(int userId, String status) throws SQLException {
        String sql = "SELECT COUNT(*) FROM returnrequest WHERE userId = ? AND status = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, userId);
